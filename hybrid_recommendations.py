...
class HybridRecommenderSystem:
    ...
    def __calculate_collaborative_filtering_similarities(self, song_name, artist_name, track_ids, songs_data, interaction_matrix):
        ...
        # index value of track_id in the full interaction matrix
        ind = np.where(track_ids == input_track_id)[0].item()
        # fetch the input vector
        input_array = interaction_matrix[ind]
        # get similarity scores against all track rows
        collaborative_similarity_scores = cosine_similarity(input_array, interaction_matrix)
        return collaborative_similarity_scores

    def __align_interaction_data(self, songs_data, track_ids, interaction_matrix):
        filtered_track_ids = songs_data['track_id'].astype(str).values
        track_ids_map = {str(tid): idx for idx, tid in enumerate(track_ids)}
        aligned_indices = [track_ids_map[tid] for tid in filtered_track_ids if tid in track_ids_map]
        aligned_track_ids = track_ids[aligned_indices]
        aligned_interaction_matrix = interaction_matrix[aligned_indices]
        return aligned_track_ids, aligned_interaction_matrix
    ...
    def give_recommendations(self, song_name, artist_name, songs_data, track_ids, transformed_matrix, interaction_matrix):
        # align collaborative data to the filtered songs list
        aligned_track_ids, aligned_interaction_matrix = self.__align_interaction_data(songs_data, track_ids, interaction_matrix)
        ...
        collaborative_filtering_similarities = self.__calculate_collaborative_filtering_similarities(song_name= song_name, 
                                                                                                   ...
                                                                                                   songs_data= songs_data, 
                                                                                                   interaction_matrix= aligned_interaction_matrix)
        ...
        # get top k recommendations from aligned track IDs
        recommendation_track_ids = aligned_track_ids[recommendation_indices]
